...
def registro():
    ...
    if request.method == 'POST':
        ...
        campos_validos, campos_vazios = validar_campos_obrigatorios(
            [username, email, nome],
            ['nome de usuário', 'email', 'nome completo']
        )
        
        if not campos_validos:
            flash('Por favor, preencha todos os campos obrigatórios', 'danger')
            return render_template('auth/registro.html', unidades=UNIDADES_DISPONIVEIS)
        ...
        # Processar registro
        sucesso = processar_solicitacao_registro(nome, username, email, setor, unidade, justificativa)
        
        if sucesso:
            flash("""Solicitação de registro enviada com sucesso! 
                  Um administrador irá analisar sua solicitação e você receberá um email com as instruções de acesso.""", 
                  "success")
            return redirect(url_for('auth.login'))
        else:
...
def processar_solicitacao_registro(nome, username, email, setor, unidade, justificativa):
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                ...
                (nome, username, email, setor, unidade, justificativa, data_solicitacao)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (nome, username, email, setor, unidade, justificativa, 
