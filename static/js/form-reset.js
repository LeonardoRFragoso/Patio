/* Copiado de bkp/form-reset.js em 2025-07-11. */
/**
 * form-reset.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function limparFormularioMovimentacao(forceClean = false) {
        ...
        // O formulário real possui o ID 'formMovimentacao'
        ...
        if (formMovimentacao) {
            ...
            const camposMovimentacao = [
                ...
                'posicao_nova',             // Nova posição
                'observacoes_movimentacao'   // Observações
            ];
        ...
        return originalFetch(url, options).then(response => {
            ...
            if (url.includes('/operacoes/')) {
                clonedResponse.json().then(data => {
                    // Verificar se o endpoint realmente registra uma operação (descarga, movimentacao, carregamento)
                    const isEndpointOperacaoReal = !url.includes('/posicoes/') && !url.includes('/containers/lista');
                    if (data && data.success === true && isEndpointOperacaoReal) {
                        ...
                        if (url.includes('/buscar_container') || url.includes('/posicoes/')) {
